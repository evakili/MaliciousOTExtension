...
void CBitVector::Create(int bits)
{
	...
	m_pBits = (BYTE*) calloc(m_nSize, sizeof(BYTE));// * m_nSize);
	//m_pBits = (BYTE*) malloc(m_nSize *  sizeof(BYTE));// * m_nSize);
	assert(m_pBits != NULL);
...
void CBitVector::XORBytes(BYTE* p, int pos, int len)
{
	...
	/*if((pos + len) > m_nSize) {
		cerr << "Error Trying to xor at byte position: " << pos << " with len = " << len << " with m_nSize = " << m_nSize << endl;
		exit(0);
	}*/
...
{
	if(vec.GetSize() * 8 < to || m_nSize * 8 < to || from > to)
		return false;
