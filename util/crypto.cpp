...
/* elliptic curve - point of prime order (x,y) , with order ecpr160*/
...
BOOL GMPInit(SECLVL lvl, BYTE* seed, fparams* params) {
	...
	params->numbytelen = params->elebytelen;
...
SECLVL getSecurityLevel(int symbits) {
	if(symbits == ST.symbits) return ST;
	else if(symbits == MT.symbits) return MT;
	else if(symbits == LT.symbits) return LT;
	else if(symbits == XLT.symbits) return XLT;
	else if(symbits == XXLT.symbits) return XXLT;
	else return LT;
}
...
/*	switch(lvl.ecckcbits)
	...
	char *ecp = NULL, *ecb = NULL, *ecx = ecx160, *ecy = ecy160, *order;
...
#ifdef OTEXT_USE_PRIMEFIELD
	mip->IOBASE = 16;
	*(params->eccparams.BA) = -3;
	*(params->eccparams.BB) = ecb;
	*(params->eccparams.BP) = ecp;
	ecurve(*(params->eccparams.BA), *(params->eccparams.BB), *(params->eccparams.BP), MR_BEST);
#else
	...
	params->eccparams.order = new Big();
