...
public:
	...
	void SetBit(int idx, BYTE b) {	m_pBits[idx>>3] = (m_pBits[idx>>3] & CMASK_BIT[idx & 0x7]) | MASK_SET_BIT_C[!(b&0x01)][idx & 0x7];	}
	void XORBit(int idx, BYTE b) {	m_pBits[idx>>3] ^= MASK_SET_BIT_C[!(b&0x01)][idx & 0x7]; }
	...
	void SetBitNoMask(int idx, BYTE b) {	m_pBits[idx>>3] = (m_pBits[idx>>3] & C_BIT[idx & 0x7]) | SET_BIT_C[!(b&0x01)][idx & 0x7];	}
	void XORBitNoMask(int idx, BYTE b) {	m_pBits[idx>>3] ^= SET_BIT_C[!(b&0x01)][idx & 0x7]; }
